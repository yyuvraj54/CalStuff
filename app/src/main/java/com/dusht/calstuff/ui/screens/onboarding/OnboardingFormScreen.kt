...
fun OnboardingFormScreen(
    ...
    val stepTitle = stepTitleFor(state.stepIndex)
    val continueLabel = if (state.stepIndex == ProfileOnboardingViewModel.TOTAL_STEPS - 1) {
    ...
    Scaffold(
        ...
        bottomBar = {
            Surface(tonalElevation = 3.dp) {
                PrimaryStickyButton(
                    text = continueLabel,
                    enabled = profileVm.isCurrentStepValid(),
                    onClick = {
        ...
        OnboardingShellLayout(
            ...
            when (state.stepIndex) {
                0 -> NameStep(
                    value = state.name,
                    onValueChange = profileVm::updateName,
                )
                1 -> AgeStep(
                    value = state.ageText,
                    onValueChange = profileVm::updateAge,
                )
                2 -> GenderStep(
                    selected = state.gender,
                    onSelect = profileVm::selectGender,
                )
                3 -> HeightStep(
                    value = state.heightCmText,
                    onValueChange = profileVm::updateHeightCm,
                )
                4 -> WeightStep(
                    value = state.weightKgText,
                    onValueChange = profileVm::updateWeightKg,
                )
                5 -> ActivityStep(
                    selected = state.activity,
                    onSelect = profileVm::selectActivity,
                )
            }
        }
    }
...
private fun ActivityStep(
    selected: ProfileActivityLevel?,
    onSelect: (ProfileActivityLevel) -> Unit,
) {
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
